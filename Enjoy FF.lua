gg.alert("🤡🤡 EnjoyInWorld\n🤡🤡 Free Fire Attackers\n🤡🤡 Happy Chiter 🤡🤡","🤡🤡 W E L L C O M E 🤡🤡")

function Main()
menu = gg.choice({
'👽 CHARACKTER 👽',

'👽 MENU AIM 👽',

'👽 MENU X 👽',

'👽 MENU SPESIAL 👽',

'👽 Exit 👽'},
nil,'👽👽Benny Zilvers\n FreeFire Attackers 😋😋')
if menu == 1 then A() end

if menu == 2 then B() end

if menu == 3 then C() end

if menu == 4 then D() end

if menu == 5 then Exit() end

XGCK=-1

end

function A()

menu1 = gg.multiChoice({

'🌝 °Antena Head 🌝',

'🌝 °Antena Hand 🌝',

'🌝 °White Body 🌝',

'🌝 °White Body Permanent 🌝',

'🌝 °Green Body 🌝',

'🌝 °Fast Medkit 🌝',

'🌝 °ReD Body🌝',

'🌝 °YelloW BoddY🌝',

'°Back Menu'},
nil,'🤡🤡 By Benny ZIlvers\n HP/WA : 0821-7535-5591 😋😋')
if menu1 == nil then else

if menu1[1] == true then a1() end

if menu1[2] == true then a2() end

if menu1[3] == true then a3() end

if menu1[4] == true then a4() end

if menu1[5] == true then a5() end

if menu1[6] == true then a6() end

if menu1[7] == true then a7() end

if menu1[8] == true then a8() end

if menu1[9] == true then HOME() end

end

GLWW=-1

end

function B()

menu1 = gg.multiChoice({

'😈😈 °Aim BoT 😈😈',

'😈😈 °Aim HS 😈😈',

'😈😈 °AutoHS Flat 😈😈',

'😈😈 °Aim LocK 😈😈',

'😈😈 °HEADSHOT 😈😈',

'😈😈 °Fast Fire 😈😈',

'😈😈 °Fast Bullet 😈😈',

'😈😈 °Fast Reload 😈😈',

'😈😈 °Fast Change Weapon 😈😈',

'😈😈 °Rain Bulets 😈😈',

'😈😈 °No Delay Fire 😈😈',

'😈😈 °Long Ammo 😈😈',

'😈😈 °Rain Fire 😈😈',

'°Back Menu'},
nil,'🤡🤡 By Benny ZIlvers\n HP/WA : 0821-7535-5591 😋😋')
if menu1 == nil then else
if menu1[1] == true then b1() end
if menu1[2] == true then b2() end
if menu1[3] == true then b3() end
if menu1[4] == true then b4() end
if menu1[5] == true then b5() end
if menu1[6] == true then b6() end
if menu1[7] == true then b7() end
if menu1[8] == true then b8() end
if menu1[9] == true then b9() end
if menu1[10] == true then b10() end
if menu1[11] == true then b11() end
if menu1[12] == true then b12() end
if menu1[13] == true then b13() end
if menu1[14] == true then HOME() end

end

GLWW=-1

end

function C()

menu1 = gg.multiChoice({

'🤡 Bypass Pinger 0 😋😋',

'🤡 Bypass Anti FC 😋😋',

'🤡 No tree & Gras 😋😋',

'🤡 Night Mode 🤡',

'🤡 Sunny Mode 🤡',

'🤡 Anti Parasute 🤡',

'🤡 Anti Report 🤡',

'°Back Menu'},
nil,'🤡🤡 By Benny ZIlvers\n HP/WA : 0821-7535-5591 😋😋')
if menu1 == nil then else
if menu1[1] == true then c1() end
if menu1[2] == true then c2() end
if menu1[3] == true then c3() end
if menu1[4] == true then c4() end
if menu1[5] == true then c5() end
if menu1[6] == true then c6() end
if menu1[7] == true then c7() end
if menu1[8] == true then HOME() end

end

GLWW=-1

end

function D()

menu1 = gg.multiChoice({

'🤡 No Eliminasi ON 🤡',

'🤡 No Eliminasi OFF 🤡',

'🌝 °Mode Ghost ON 🌝',

'🌝 °Mode Ghost OFF 🌝',

'🌝 °Bug Batu ON 🌝',

'🌝 °Bug Batu OFF 🌝',

'🤡 Walhack ON 🤡',

'🤡 Walhack OFF 🤡',

'🤡 Speed Run 5x ON 🤡',

'🤡 Speed Run 5x OFF 🤡',

'🤡 Eagle Eye ON 🤡',

'🤡 Eagle Eye OFF 🤡',

'°Back Menu'},
nil,'🤡🤡 By Benny ZIlvers\n HP/WA : 0821-7535-5591 😋😋')
if menu1 == nil then else

if menu1[1] == true then d1() end

if menu1[2] == true then d2() end

if menu1[3] == true then d3() end

if menu1[4] == true then d4() end

if menu1[5] == true then d5() end

if menu1[6] == true then d6() end

if menu1[7] == true then d7() end

if menu1[8] == true then d8() end

if menu1[9] == true then d9() end

if menu1[10] == true then d10() end

if menu1[11] == true then d11() end

if menu1[12] == true then d12() end

if menu1[13] == true then HOME() end

end

GLWW=-1

end

function a1()
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("7.5538861e-7F;1F:5", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("1", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100)
gg.editAll("200", gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("5.9762459e-7F;1F:5", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("1", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100)
gg.editAll("200", gg.TYPE_FLOAT)
gg.toast("Antena Head Aktif!!!")
end

function a2()
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("1;3.2404066e-7::5", gg.TYPE_FLOAT)
gg.getResults(1000)
gg.editAll("250.1;3.2404066e-7", gg.TYPE_FLOAT)
gg.clearResults()
gg.searchNumber("-0.39830258489;1;0.5870424509::9", gg.TYPE_FLOAT)
gg.getResults(1000)
gg.editAll("-0.39830258489;250.1;0.5870424509", gg.TYPE_FLOAT)
gg.toast("Antena Hand Aktif!!!")
end

function a3()
gg.clearResults()
gg.setRanges(gg.REGION_C_HEAP | gg.REGION_C_ALLOC)
gg.searchNumber("0000000ch;00000031h;000000bdh;40800000h::29", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.searchNumber("40800000h", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1000)
gg.editAll("44613f36h", gg.TYPE_DWORD)
gg.clearResults()
gg.toast("👰WHITE BODY👰")
end

function a4()
gg.clearResults()
gg.setRanges(gg.REGION_CODE_APP)
gg.getRangesList("libil2cpp.so")
gg.searchNumber("7Fr;45r;4Cr;46r;01r;01r;01r;00r::8", gg.TYPE_BYTE, false, gg.SIGN_EQUAL, 0x8ED1C000, 0x9210F000)
gg.getResultCount()
gg.getResults(8)
gg.loadResults({ -- table(535dc428)
[1] = { -- table(535dc510)
['address'] = 0x8f7473a4,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 127,
},
[2] = { -- table(535dc558)
['address'] = 0x8f7473a5,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 69,
},
[3] = { -- table(535dc5a0)
['address'] = 0x8f7473a6,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 76,
},
[4] = { -- table(535dc5e8)
['address'] = 0x8f7473a7,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 70,
},
[5] = { -- table(535dc630)
['address'] = 0x8f7473a8,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 1,
},
[6] = { -- table(535dc678)
['address'] = 0x8f7473a9,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 1,
},
[7] = { -- table(53532a4c)
['address'] = 0x8f7473aa,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 1,
},
[8] = { -- table(53532a94)
['address'] = 0x8f7473ab,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 0,
},
})
gg.setValues({ -- table(53621450)
[1] = { -- table(536214c8)
['address'] = 0x8f7473a4,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '01r',
},
[2] = { -- table(536215a4)
['address'] = 0x8f7473a5,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '00r',
},
[3] = { -- table(5362162c)
['address'] = 0x8f7473a6,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'A0r',
},
[4] = { -- table(5362170c)
['address'] = 0x8f7473a7,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'E3r',
},
[5] = { -- table(53621794)
['address'] = 0x8f7473a8,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '1Er',
},
[6] = { -- table(5362187c)
['address'] = 0x8f7473a9,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'FFr',
},
[7] = { -- table(53621904)
['address'] = 0x8f7473aa,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '2Fr',
},
[8] = { -- table(5362198c)
['address'] = 0x8f7473ab,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'E1r',
},
})
gg.clearResults()
gg.searchNumber("7Fr;45r;4Cr;46r;01r;01r;01r;00r::8", gg.TYPE_BYTE, false, gg.SIGN_EQUAL, 0x8ED1C000, 0x9210F000)
gg.getResultCount()
gg.getResults(8)
gg.loadResults({ -- table(52a4e45c)
[1] = { -- table(52a4e570)
['address'] = 0x901e414c,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 127,
},
[2] = { -- table(52a4e5b8)
['address'] = 0x901e414d,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 69,
},
[3] = { -- table(52a4e600)
['address'] = 0x901e414e,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 76,
},
[4] = { -- table(52a4e648)
['address'] = 0x901e414f,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 70,
},
[5] = { -- table(52a4e690)
['address'] = 0x901e4150,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 1,
},
[6] = { -- table(52a4e6d8)
['address'] = 0x901e4151,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 1,
},
[7] = { -- table(52a4e720)
['address'] = 0x901e4152,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 1,
},
[8] = { -- table(52a4e768)
['address'] = 0x901e4153,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 0,
},
})
gg.setValues({ -- table(53619f30)
[1] = { -- table(53614cdc)
['address'] = 0x901e414c,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '00r',
},
[2] = { -- table(5361375c)
['address'] = 0x901e414d,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '00r',
},
[3] = { -- table(535e5fe0)
['address'] = 0x901e414e,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'BBr',
},
[4] = { -- table(5357f14c)
['address'] = 0x901e414f,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'E3r',
},
})
gg.toast("🅐🅒white body permanent⚪️")
end

function a5()
gg.clearResults()
gg.setRanges(gg.REGION_BAD)
gg.searchNumber("1,194,363,394;589,832;655,368::557", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.searchNumber("1,194,363,394", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(10)
gg.editAll("1,102,363,394", gg.TYPE_DWORD)
gg.clearResults()
gg.setRanges(gg.REGION_BAD)
gg.searchNumber("1,102,363,394;589,832;655,368::557", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.searchNumber("589,832;655,368", gg.TYPE_DWORD, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(10)
gg.editAll("111111;666666", gg.TYPE_DWORD)
gg.clearResults()
gg.toast("🐢GREEN BODY🐢")
gg.setVisible(false)
gg.toast("SUBSCRIBE🔴")
end

function a6()
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
      gg.searchNumber("4;0;4.2038954e-45::9", gg.TYPE_FLOAT)
      gg.getResults(1)
      gg.editAll("3", gg.TYPE_FLOAT)
      gg.toast("Medkit 3 Second New Aktiff!!!")
end

function a7()
gg.setRanges(gg.REGION_BAD)
gg.searchNumber("1,194,363,394;589,832;655,368::557", 4, false, 536870912, 0, -1)
gg.searchNumber("1,194,363,394", 4, false, 536870912, 0, -1)
gg.getResults(10)
gg.editAll("1,102,363,394", 4)
gg.clearResults()
gg.setRanges(gg.REGION_BAD)
gg.searchNumber("1,102,363,394;589,832;655,368::557", 4, false, 536870912, 0, -1)
gg.searchNumber("589,832;655,368", 4, false, 536870912, 0, -1)
gg.getResults(10)
gg.editAll("666666;50555555", 4)
gg.clearResults()
gg.toast("ReD BoddY AktiFF")
gg.setVisible(false)
gg.toast("SUBSCRIBE Benny Zilvers🔴")
end

function a8()
gg.setRanges(gg.REGION_BAD)
gg.searchNumber("1,194,363,394;589,832;655,368::557", 4, false, 536870912, 0, -1)
gg.searchNumber("1,194,363,394", 4, false, 536870912, 0, -1)
gg.getResults(10)
gg.editAll("1,102,363,394", 4)
gg.clearResults()
gg.setRanges(gg.REGION_BAD)
gg.searchNumber("1,102,363,394;589,832;655,368::557", 4, false, 536870912, 0, -1)
gg.searchNumber("589,832;655,368", 4, false, 536870912, 0, -1)
gg.getResults(10)
gg.editAll("666666", 4)
gg.clearResults()
gg.toast("YelloW BodY AktiFF")
gg.setVisible(false)
gg.toast("SUBSCRIBE Benny Zilvers🔴")
end

function b1()
gg.clearResults()

gg.setRanges(16392)

gg.getRangesList("libil2cpp.so")

gg.searchNumber("7Fr;45r;4Cr;46r;01r;01r;01r;00r::8", 1, false, 536870912, 3368075264, 3420114944)

gg.getResultsCount()

gg.getResults(8)

gg.loadResults({ -- table(af90476)

[1] = { -- table(4a70a77)

['address'] = 0xca7d0d2c,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 127,

},

[2] = { -- table(f59e6e4)

['address'] = 0xca7d0d2d,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 69,

},

[3] = { -- table(5f8f74d)

['address'] = 0xca7d0d2e,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 76,

},

[4] = { -- table(8e8f202)

['address'] = 0xca7d0d2f,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 70,

},

[5] = { -- table(477d713)

['address'] = 0xca7d0d30,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[6] = { -- table(6408d50)

['address'] = 0xca7d0d31,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[7] = { -- table(6abb49)

['address'] = 0xca7d0d32,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[8] = { -- table(5f82c4e)

['address'] = 0xca7d0d33,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 0,

},

})

gg.setValues({ -- table(21ac305)

[1] = { -- table(7aebf5a)

['address'] = 0xca7d0d2c,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '5Cr',

},

[2] = { -- table(d51158b)

['address'] = 0xca7d0d2d,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '04r',

},

[3] = { -- table(498b668)

['address'] = 0xca7d0d2e,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '44r',

},

[4] = { -- table(9098a81)

['address'] = 0xca7d0d2f,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'E3r',

},

[5] = { -- table(2097726)

['address'] = 0xca7d0d30,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '1Er',

},

[6] = { -- table(db97f67)

['address'] = 0xca7d0d31,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'FFr',

},

[7] = { -- table(d76d114)

['address'] = 0xca7d0d32,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '2Fr',

},

[8] = { -- table(8c34dbd)

['address'] = 0xca7d0d33,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'E1r',

},

})
gg.clearResults()
gg.toast("AIMBOT Aktiff🎯")
os.remove("/storage/emulated/0/Android/data/com.dts.freefireth/files/reportnew.db")
os.remove("/storage/emulated/0/Android/data/com.dts.freefireth/files/ymrtc_log.txt")
os.remove("/storage/emulated/0/á´�á´�Ê�á´�á´�á´�á´�É¢á´�á´�á´�Ê�.Ê�á´�á´�")
gg.setVisible(false)
end

function b2()
gg.clearResults()
gg.setRanges(32)
gg.searchNumber("0000B040rA;0000803FrA;0000403FrA:9", 4, false, 536870912, 0, -1)
gg.getResults(3)
gg.editAll("1,085,276,160;0;0", 4)
gg.toast("Head SHot +")
end

function b3()
gg.clearResults()
gg.setRanges(gg.REGION_C_DATA)
gg.searchNumber("-360;40::5",gg.TYPE_FLOAT)
gg.getResults(2)
gg.editAll("-360;0",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_C_DATA)
gg.searchNumber("-360;40::5",gg.TYPE_FLOAT)
gg.getResults(2)
gg.editAll("-360;0",gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_C_DATA)
gg.searchNumber("-360;40::5",gg.TYPE_FLOAT)
gg.getResults(2)
gg.editAll("-360;0",gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("Auto Hedshot Flat")
end

function b4()
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("�00r;00r;00r;3Fr;00r;00r;80r;3Fr;00r;00r;00r;00r;00r;00r;00r;00r;00r;00r;00r;3Fr;00r;00r;00r;00r;00r;00r;80r;3Fr;D9r;D8r;58r;3Fr::32", gg.TYPE_BYTE)
gg.getResults(10)
gg.editAll("00r;00r;20r;41r", gg.TYPE_BYTE)
gg.clearResults()
gg.toast("🔐AIM LOCK🔐")
end

function b5()
gg.clearResults()
gg.setRanges(32)

gg.searchNumber("0000B040rA;0000803FrA;0000403FrA:9", 4, false, 536870912, 0, -1)

gg.getResults(3)

gg.editAll("1,085,276,160;0;0", 4)

gg.clearResults()

gg.toast("CARGANDO🔥")

gg.setRanges(16384)

gg.searchNumber("7Fr;45r;4Cr;46r;01r;01r;01r;00r::8", 1, false, 536870912, 3368075264, 3420114944)

gg.getResultsCount()

gg.getResults(8)

gg.loadResults({ -- table(da89745)

[1] = { -- table(33d9e9a)

['address'] = 0xca7d135c,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 127,

},

[2] = { -- table(caba3cb)

['address'] = 0xca7d135d,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 69,

},

[3] = { -- table(afec7a8)

['address'] = 0xca7d135e,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 76,

},

[4] = { -- table(34062c1)

['address'] = 0xca7d135f,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 70,

},

[5] = { -- table(2c70a66)

['address'] = 0xca7d1360,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[6] = { -- table(4ceb1a7)

['address'] = 0xca7d1361,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[7] = { -- table(44cb654)

['address'] = 0xca7d1362,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[8] = { -- table(d2f69fd)

['address'] = 0xca7d1363,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 0,

},

})

gg.setValues({ -- table(e0da8f9)

[1] = { -- table(ee3003e)

['address'] = 0xca7d135c,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '10r',

},

[2] = { -- table(eae2e9f)

['address'] = 0xca7d135d,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '00r',

},

[3] = { -- table(587bec)

['address'] = 0xca7d135e,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'A0r',

},

[4] = { -- table(ff6dbb5)

['address'] = 0xca7d135f,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'E3r',

},

[5] = { -- table(60ae24a)

['address'] = 0xca7d1360,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '1Er',

},

[6] = { -- table(1476ad8)

['address'] = 0xca7d1361,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'FFr',

},

[7] = { -- table(9067e31)

['address'] = 0xca7d1362,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '2Fr',

},

[8] = { -- table(d1cd916)

['address'] = 0xca7d1363,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'E1r',

},

})
gg.clearResults()
gg.toast("HEADSHOT ON☠")
end

function b6()
gg.clearResults()
gg.setRanges(gg.REGION_C_ALLOC)
gg.searchNumber("0.05", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(80000)
gg.editAll("55.0", gg.TYPE_FLOAT)
gg.toast("🔺BIG EFFECT FIRE🔺")
end

function b7()
gg.clearResults()
gg.setRanges(gg.REGION_C_DATA)
gg.searchNumber("-8.5003245e22;-2.0291021e20;-8.5004722e22:89", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.searchNumber("-8.5003245e22;-2.0291021e20", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(2)
gg.editAll("-3.5981150e21;-2.0291021e20", gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_C_DATA)
gg.searchNumber("0.20000000298", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(30)
gg.editAll("0", gg.TYPE_FLOAT)
gg.toast("🔫FAST BULLET🔫")
end

function b8()
gg.clearResults()
gg.setRanges(gg.REGION_C_DATA | gg.REGION_CODE_APP)
gg.searchNumber("7Fr;45r;4Cr;46r;01r;01r;01r;00r::8", gg.TYPE_BYTE, false, gg.SIGN_EQUAL, 0x4000000, 0x6E11000)
gg.getResultCount()
gg.getResults(8)
gg.loadResults({ -- table(52950db4)
[1] = { -- table(52a4a438)
['address'] = 0x571f08c,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 127,
},
[2] = { -- table(534956d4)
['address'] = 0x571f08d,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 69,
},
[3] = { -- table(5349570c)
['address'] = 0x571f08e,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 76,
},
[4] = { -- table(53495750)
['address'] = 0x571f08f,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 70,
},
[5] = { -- table(53495794)
['address'] = 0x571f090,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 1,
},
[6] = { -- table(534a6d0c)
['address'] = 0x571f091,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 1,
},
[7] = { -- table(534a6d44)
['address'] = 0x571f092,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 1,
},
[8] = { -- table(534a6d88)
['address'] = 0x571f093,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 0,
},
})
gg.setValues({ -- table(53557920)
[1] = { -- table(53557998)
['address'] = 0x571f08c,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '12r',
},
[2] = { -- table(529980c4)
['address'] = 0x571f08d,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '03r',
},
[3] = { -- table(52998148)
['address'] = 0x571f08e,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'A0r',
},
[4] = { -- table(52a2e55c)
['address'] = 0x571f08f,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'E3r',
},
[5] = { -- table(52a2e5e0)
['address'] = 0x571f090,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '1Er',
},
[6] = { -- table(52a2fe90)
['address'] = 0x571f091,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'FFr',
},
[7] = { -- table(52a2ff14)
['address'] = 0x571f092,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '2Fr',
},
[8] = { -- table(534bd244)
['address'] = 0x571f093,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'E1r',
},
})
gg.toast("ᴏFast reload✔")
end

function b9()
gg.clearResults()
gg.setRanges(gg.REGION_C_DATA | gg.REGION_CODE_APP)
gg.searchNumber("7Fr;45r;4Cr;46r;01r;01r;01r;00r::8", gg.TYPE_BYTE, false, gg.SIGN_EQUAL, 0x4000000, 0x6E11000)
gg.getResultCount()
gg.getResults(8)
gg.loadResults({ -- table(52a1e304)
[1] = { -- table(52a1e3ec)
['address'] = 0x4a4fe14,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 127,
},
[2] = { -- table(52a1e430)
['address'] = 0x4a4fe15,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 69,
},
[3] = { -- table(52a1e474)
['address'] = 0x4a4fe16,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 76,
},
[4] = { -- table(52a1e4b8)
['address'] = 0x4a4fe17,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 70,
},
[5] = { -- table(52a1e4fc)
['address'] = 0x4a4fe18,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 1,
},
[6] = { -- table(52a1e540)
['address'] = 0x4a4fe19,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 1,
},
[7] = { -- table(52a1e584)
['address'] = 0x4a4fe1a,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 1,
},
[8] = { -- table(52a1e5c8)
['address'] = 0x4a4fe1b,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 0,
},
})
gg.setValues({ -- table(52cdf990)
[1] = { -- table(52cdfa08)
['address'] = 0x4a4fe14,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '00r',
},
[2] = { -- table(52cdfad0)
['address'] = 0x4a4fe15,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '00r',
},
[3] = { -- table(52cdfb44)
['address'] = 0x4a4fe16,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'A0r',
},
[4] = { -- table(534d3b5c)
['address'] = 0x4a4fe17,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'E3r',
},
[5] = { -- table(534d3bd0)
['address'] = 0x4a4fe18,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '1Er',
},
[6] = { -- table(534d3ca4)
['address'] = 0x4a4fe19,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'FFr',
},
[7] = { -- table(534d3d28)
['address'] = 0x4a4fe1a,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '2Fr',
},
[8] = { -- table(534d3dac)
['address'] = 0x4a4fe1b,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'E1r',
},
})
gg.toast("ᴏfast change weapon")
end

function b10()
gg.clearResults()
gg.setRanges(gg.REGION_CODE_APP)
gg.getRangesList("libil2cpp.so")
gg.searchNumber("7Fr;45r;4Cr;46r;01r;01r;01r;00r::8", gg.TYPE_BYTE, false, gg.SIGN_EQUAL, 0x8EAC6000, 0x91EAD000)
gg.getResultCount()
gg.getResults(8)
gg.loadResults({ -- table(5367fff8)
[1] = { -- table(536800e8)
['address'] = 0x901d753c,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 127,
},
[2] = { -- table(53680130)
['address'] = 0x901d753d,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 69,
},
[3] = { -- table(53680178)
['address'] = 0x901d753e,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 76,
},
[4] = { -- table(536801c0)
['address'] = 0x901d753f,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 70,
},
[5] = { -- table(53680208)
['address'] = 0x901d7540,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 1,
},
[6] = { -- table(53680250)
['address'] = 0x901d7541,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 1,
},
[7] = { -- table(53680298)
['address'] = 0x901d7542,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 1,
},
[8] = { -- table(536802e0)
['address'] = 0x901d7543,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 0,
},
})
gg.setValues({ -- table(534ad8ec)
[1] = { -- table(534ad964)
['address'] = 0x901d753c,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '18r',
},
[2] = { -- table(534ada40)
['address'] = 0x901d753d,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '00r',
},
[3] = { -- table(534adab8)
['address'] = 0x901d753e,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'A0r',
},
[4] = { -- table(536f2c58)
['address'] = 0x901d753f,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'E3r',
},
[5] = { -- table(536f2ce0)
['address'] = 0x901d7540,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '1Er',
},
[6] = { -- table(536f2dc8)
['address'] = 0x901d7541,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'FFr',
},
[7] = { -- table(536f2e50)
['address'] = 0x901d7542,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '2Fr',
},
[8] = { -- table(536f2ed8)
['address'] = 0x901d7543,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'E1r',
},
})
gg.clearResults()
gg.toast("🅐🅒hujan peluru🅞 💥")
end

function b11()
gg.clearResults()
gg.setRanges(gg.REGION_C_DATA)
gg.searchNumber("0.20000000298;10;2:9", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("0.20000000298", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(2)
gg.editAll("0", gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_C_DATA)
gg.searchNumber("0.20000000298;-1.3092761e25;-3.7052061e20:9", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("0.20000000298", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(2)
gg.editAll("0", gg.TYPE_FLOAT)
gg.clearResults()
gg.setRanges(gg.REGION_C_DATA)
gg.searchNumber("-3.8713633e25;0.20000000298;-1.309396e25:9", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("0.20000000298", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(2)
gg.editAll("0", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🔥NO FIRE DELAY🔥")
end

function b12()
gg.clearResults()
gg.setRanges(48)
gg.searchNumber("6D;0.40000000596;0.4000000059615;5;10::65", 16, false, 536870912, 0, -1)
gg.searchNumber("6", 4, false, 536870912, 0, -1)
gg.getResults(8)
gg.editAll("99999", 4)
gg.clearResults()
gg.setRanges(48)
gg.searchNumber("30D;0.18000000715;0.18000000715;220~225;40;90::65", 16, false, 536870912, 0, -1)
gg.searchNumber("30", 4, false, 536870912, 0, -1)
gg.getResults(8)
gg.editAll("99999", 4)
gg.clearResults()
gg.setRanges(48)
gg.searchNumber("30D;0.1099999994;0.1099999994;220~225;13;30::65", 16, false, 536870912, 0, -1)
gg.searchNumber("30", 4, false, 536870912, 0, -1)
gg.getResults(8)
gg.editAll("99999", 4)
gg.clearResults()
gg.setRanges(48)
gg.searchNumber("20D;0.0700000003;0.0700000003;220~225;11;25::65", 16, false, 536870912, 0, -1)
gg.searchNumber("20", 4, false, 536870912, 0, -1)
gg.getResults(8)
gg.editAll("99999", 4)
gg.clearResults()
gg.setRanges(48)
gg.searchNumber("30D;0.15000000596;0.15000000596;220~225;30;70::65", 16, false, 536870912, 0, -1)
gg.searchNumber("30", 4, false, 536870912, 0, -1)
gg.getResults(8)
gg.editAll("99999", 4)
gg.clearResults()
gg.setRanges(48)
gg.searchNumber("30D;0.18000000715;0.18000000715;220~225;38;85::65", 16, false, 536870912, 0, -1)
gg.searchNumber("30", 4, false, 536870912, 0, -1)
gg.getResults(8)
gg.editAll("99999", 4)
gg.clearResults()
gg.setRanges(48)
gg.searchNumber("30D;0.09000000358;0.09000000358;220~225;16;34::65", 16, false, 536870912, 0, -1)
gg.searchNumber("30", 4, false, 536870912, 0, -1)
gg.getResults(8)
gg.editAll("99999", 4)
gg.clearResults()
gg.setRanges(48)
gg.searchNumber("30D;0.11999999732;0.15000000596;220~225;38;80::65", 16, false, 536870912, 0, -1)
gg.searchNumber("30", 4, false, 536870912, 0, -1)
gg.getResults(8)
gg.editAll("99999", 4)
gg.clearResults()
gg.setRanges(48)
gg.searchNumber("50D;0.15999999642;0.15999999642;220~225;45;80::65", 16, false, 536870912, 0, -1)
gg.searchNumber("50", 4, false, 536870912, 0, -1)
gg.getResults(8)
gg.editAll("99999", 4)
gg.clearResults()
gg.setRanges(48)
gg.searchNumber("50D;0.33000001311;0.33000001311;25;4;20::65", 16, false, 536870912, 0, -1)
gg.searchNumber("50", 4, false, 536870912, 0, -1)
gg.getResults(8)
gg.editAll("99999", 4)
gg.clearResults()
gg.setRanges(48)
gg.searchNumber("50D;0.09000000358;0.09000000358;220~225::21", 16, false, 536870912, 0, -1)
gg.searchNumber("50", 4, false, 536870912, 0, -1)
gg.getResults(8)
gg.editAll("99999", 4)
gg.clearResults()
gg.toast("⛓️Long Ammo :D📥")
end

function b13()
gg.clearResults()
gg.setRanges(16392)

gg.searchNumber("7Fr;45r;4Cr;46r;01r;01r;01r;00r::8", 1, false, 536870912, 3368075264, 3420114944)

gg.getResultsCount()

gg.getResults(8)

gg.loadResults({ -- table(e5a4af6)

[1] = { -- table(f8d0ef7)

['address'] = 0xca7cce6c,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 127,

},

[2] = { -- table(3a8d164)

['address'] = 0xca7cce6d,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 69,

},

[3] = { -- table(605cfcd)

['address'] = 0xca7cce6e,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 76,

},

[4] = { -- table(b582082)

['address'] = 0xca7cce6f,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 70,

},

[5] = { -- table(4f9a393)

['address'] = 0xca7cce70,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[6] = { -- table(4d39fd0)

['address'] = 0xca7cce71,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[7] = { -- table(3ac9bc9)

['address'] = 0xca7cce72,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[8] = { -- table(e6bc2ce)

['address'] = 0xca7cce73,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 0,

},

})

gg.clearResults()

gg.prompt({ -- table(6053dda)

[1] = 'Tiro Rapido [2;10]',

}, { -- table(b2b98e8)

[1] = 2,

}, { -- table(596fb01)

[1] = 'number',

})

--[[ LOGS BY MUHAGAMER: { -- table(60cb34a)

[1] = '9',

} ]]

gg.setValues({ -- table(74512bb)

[1] = { -- table(bc023d8)

['address'] = 0xca7cce6c,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '09r',

},

[2] = { -- table(ccb4331)

['address'] = 0xca7cce6d,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '00r',

},

[3] = { -- table(9eaba16)

['address'] = 0xca7cce6e,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'A0r',

},

[4] = { -- table(a81f97)

['address'] = 0xca7cce6f,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'E3r',

},

[5] = { -- table(6ffa584)

['address'] = 0xca7cce70,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '1Er',

},

[6] = { -- table(75fe16d)

['address'] = 0xca7cce71,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'FFr',

},

[7] = { -- table(1d061a2)

['address'] = 0xca7cce72,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '2Fr',

},

[8] = { -- table(8c51e33)

['address'] = 0xca7cce73,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'E1r',

},

})
gg.clearResults()
gg.setVisible(false)
end

function c1()
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("1.3984959e-42;0.0;1.4012985e-45;7.0064923e-45;1.4012985e-42:17", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100)
gg.editAll("0", gg.TYPE_FLOAT)
gg.toast("📶BYPASS PINGER 0📶")
end

function c2()
gg.clearResults()
gg.setRanges(gg.REGION_CODE_APP)
gg.searchNumber("-1.1888024e-10F;-0.00883197878F;-9,004,122,112.0F;4.8888483e24F;-0.0079164654F;1.0865689e-19F;1.0879452e-19F;4.1778991e34F:29", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("-1.1888024e-10;-0.00883197878", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(100)
gg.editAll("61,510.75", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🔗BYPASS NO FC🔗")
end

function c3()
gg.clearResults()
    gg.setRanges(gg.REGION_ANONYMOUS)
    gg.searchNumber("6D;100;300::17", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
    gg.searchNumber("100", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
    gg.getResults(3)
    gg.editAll("-1", gg.TYPE_FLOAT)
    gg.clearResults()
    gg.setVisible(false)
    gg.setRanges(gg.REGION_C_DATA)
    gg.searchNumber("60", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
    gg.getResults(8)
    gg.editAll("-1.12999999523", gg.TYPE_FLOAT)
    gg.clearResults()
    gg.setVisible(false)
    gg.setRanges(gg.REGION_C_DATA)
    gg.searchNumber("60", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
    gg.getResults(8)
    gg.editAll("-1.12999999523", gg.TYPE_FLOAT)
    gg.clearResults()
    gg.setVisible(false)
    gg.toast("Remove Tree & Grass Aktif!!!")
end

function c4()
gg.clearResults()
gg.setRanges(gg.REGION_C_DATA)
gg.searchNumber("0.57735025883F;1.0e-6F:9", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("1.0e-6", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1)
gg.editAll("-1", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🌃NIGHT MODE🌃")
gg.setVisible(false)
gg.setVisible(false)
gg.toast("Subscribe Youtube.com/c/bennyzilvers🔴")
end

function c5()
gg.clearResults()
gg.setRanges(gg.REGION_C_DATA)
gg.searchNumber("0.57735025883F;-1:9", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.refineNumber("-1", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(1)
gg.editAll("1.0e-6", gg.TYPE_FLOAT)
gg.clearResults()
gg.toast("🌇SUNNY MODE🌇")
gg.setVisible(false)
gg.toast("Subscribe Youtube.com/c/bennyzilvers🔴")
end

function c6()
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("-12;-4;-8::45", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.searchNumber("-8", gg.TYPE_FLOAT, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(5)
gg.editAll("-99996", gg.TYPE_FLOAT)
gg.toast("No Parachute!! Suksess")
end

function c7()
gg.clearResults()
gg.setRanges(gg.REGION_ANONYMOUS)
gg.searchNumber("4Br;00r;00r;00r;04r;00r;00r;00r;05r;00r;00r;00r;00r;00r;80r;40r::16", gg.TYPE_BYTE, false, gg.SIGN_EQUAL, 0, -1)
gg.getResults(20)
gg.editAll("4Br;00r;00r;00r;03r;00r;00r;00r;04r;00r;00r;00r;66r;66r;3Er;40r", gg.TYPE_BYTE)
gg.clearResults()
gg.toast("🚫NO REPORT🚫")
end

function d1()
gg.clearResults()

gg.setRanges(16392)

gg.getRangesList("libil2cpp.so")

gg.searchNumber("7Fr;45r;4Cr;46r;01r;01r;01r;00r::8", 1, false, 536870912, 3304169472, 3356209152)

gg.getResultsCount()

gg.getResults(8)

gg.loadResults({ -- table(9e259d2)

[1] = { -- table(72a6aa3)

['address'] = 0xc67cf5a0,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 127,

},

[2] = { -- table(69f41a0)

['address'] = 0xc67cf5a1,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 69,

},

[3] = { -- table(98de159)

['address'] = 0xc67cf5a2,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 76,

},

[4] = { -- table(fe8411e)

['address'] = 0xc67cf5a3,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 70,

},

[5] = { -- table(4ef0dff)

['address'] = 0xc67cf5a4,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[6] = { -- table(246d7cc)

['address'] = 0xc67cf5a5,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[7] = { -- table(1b23a15)

['address'] = 0xc67cf5a6,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[8] = { -- table(7d3512a)

['address'] = 0xc67cf5a7,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 0,

},

})

gg.setValues({ -- table(8308b1b)

[1] = { -- table(8efa4b8)

['address'] = 0xc67cf5a0,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '00r',

},

[2] = { -- table(896291)

['address'] = 0xc67cf5a1,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '00r',

},

[3] = { -- table(3ad55f6)

['address'] = 0xc67cf5a2,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'A0r',

},

[4] = { -- table(342bdf7)

['address'] = 0xc67cf5a3,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'E3r',

},

[5] = { -- table(d565464)

['address'] = 0xc67cf5a4,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '1Er',

},

[6] = { -- table(4d096cd)

['address'] = 0xc67cf5a5,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'FFr',

},

[7] = { -- table(a50db82)

['address'] = 0xc67cf5a6,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '2Fr',

},

[8] = { -- table(fdb4293)

['address'] = 0xc67cf5a7,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'E1r',

},

})
gg.clearResults()
gg.toast("ɴᴏ ᴇʟɪᴍɪɴᴀsɪ ᴏɴ")
end

function d2()
gg.clearResults()
gg.setRanges(16392)
gg.searchNumber("7Fr;45r;4Cr;46r;01r;01r;01r;00r::8", 1, false, 536870912, 3304169472, 3356209152)
gg.getResultsCount()
gg.getResults(8)
gg.loadResults({ -- table(535022e)
[1] = { -- table(bd021cf)
['address'] = 0xc67cf5a0,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 127,
},
[2] = { -- table(705a75c)
['address'] = 0xc67cf5a1,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 69,
},
[3] = { -- table(38e7a65)
['address'] = 0xc67cf5a2,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 76,
},
[4] = { -- table(c13133a)
['address'] = 0xc67cf5a3,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 70,
},
[5] = { -- table(cd70beb)
['address'] = 0xc67cf5a4,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 1,
},
[6] = { -- table(ff3dd48)
['address'] = 0xc67cf5a5,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 1,
},
[7] = { -- table(5f157e1)
['address'] = 0xc67cf5a6,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 1,
},
[8] = { -- table(98f2906)
['address'] = 0xc67cf5a7,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 0,
},
})
gg.setValues({ -- table(1187bc7)
[1] = { -- table(d85f4)
['address'] = 0xc67cf5a0,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'F0r',
},
[2] = { -- table(34a911d)
['address'] = 0xc67cf5a1,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '4Fr',
},
[3] = { -- table(864cf92)
['address'] = 0xc67cf5a2,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '2Dr',
},
[4] = { -- table(7170d63)
['address'] = 0xc67cf5a3,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'E9r',
},
[5] = { -- table(8770d60)
['address'] = 0xc67cf5a4,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '1Cr',
},
[6] = { -- table(9042219)
['address'] = 0xc67cf5a5,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'B0r',
},
[7] = { -- table(17c52de)
['address'] = 0xc67cf5a6,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = '8Dr',
},
[8] = { -- table(8e31cbf)
['address'] = 0xc67cf5a7,
['flags'] = 1, -- gg.TYPE_BYTE
['value'] = 'E2r',
},
})
gg.clearResults()
gg.toast("ɴᴏ ᴇʟɪᴍɪɴᴀsɪɴ ᴏғғ")
end

function d3()
gg.clearResults()
gg.setRanges(16392)

gg.getRangesList("libil2cpp.so")

gg.searchNumber("7Fr;45r;4Cr;46r;01r;01r;01r;00r::8", 1, false, 536870912, 3368075264, 3420114944)

gg.getResultsCount()

gg.getResults(8)

gg.loadResults({ -- table(28ac518)

[1] = { -- table(5b36b71)

['address'] = 0xca271cc0,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 127,

},

[2] = { -- table(2875d56)

['address'] = 0xca271cc1,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 69,

},

[3] = { -- table(89721d7)

['address'] = 0xca271cc2,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 76,

},

[4] = { -- table(7f31ac4)

['address'] = 0xca271cc3,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 70,

},

[5] = { -- table(c424dad)

['address'] = 0xca271cc4,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[6] = { -- table(cb3f8e2)

['address'] = 0xca271cc5,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[7] = { -- table(f420473)

['address'] = 0xca271cc6,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[8] = { -- table(691f30)

['address'] = 0xca271cc7,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 0,

},

})

gg.setValues({ -- table(eae74cf)

[1] = { -- table(ef8fe5c)

['address'] = 0xca271cc0,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '00r',

},

[2] = { -- table(4ac8565)

['address'] = 0xca271cc1,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '00r',

},

[3] = { -- table(f37c23a)

['address'] = 0xca271cc2,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'A0r',

},

[4] = { -- table(ac78eeb)

['address'] = 0xca271cc3,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'E3r',

},

[5] = { -- table(945a448)

['address'] = 0xca271cc4,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '1Er',

},

[6] = { -- table(26512e1)

['address'] = 0xca271cc5,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'FFr',

},

[7] = { -- table(bcfc806)

['address'] = 0xca271cc6,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '2Fr',

},

[8] = { -- table(10e2ec7)

['address'] = 0xca271cc7,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'E1r',

},

})
gg.clearResults()
gg.toast("Mode Ghost Aktiff 👻',✔")
gg.setVisible(false)
end

function d4()
gg.clearResults()
gg.setRanges(16392)

gg.searchNumber("7Fr;45r;4Cr;46r;01r;01r;01r;00r::8", 1, false, 536870912, 3368075264, 3420114944)

gg.getResultsCount()

gg.getResults(8)

gg.loadResults({ -- table(8e69957)

[1] = { -- table(d31fc44)

['address'] = 0xca271cc0,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 127,

},

[2] = { -- table(9bd512d)

['address'] = 0xca271cc1,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 69,

},

[3] = { -- table(a257662)

['address'] = 0xca271cc2,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 76,

},

[4] = { -- table(dd173f3)

['address'] = 0xca271cc3,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 70,

},

[5] = { -- table(47498b0)

['address'] = 0xca271cc4,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[6] = { -- table(44ad329)

['address'] = 0xca271cc5,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[7] = { -- table(ae896ae)

['address'] = 0xca271cc6,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[8] = { -- table(b855c4f)

['address'] = 0xca271cc7,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 0,

},

})

gg.setValues({ -- table(eab6fba)

[1] = { -- table(ee56e6b)

['address'] = 0xca271cc0,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '70r',

},

[2] = { -- table(116cdc8)

['address'] = 0xca271cc1,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '4Cr',

},

[3] = { -- table(a18be61)

['address'] = 0xca271cc2,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '2Dr',

},

[4] = { -- table(690cd86)

['address'] = 0xca271cc3,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'E9r',

},

[5] = { -- table(5ea8647)

['address'] = 0xca271cc4,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '10r',

},

[6] = { -- table(baefe74)

['address'] = 0xca271cc5,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'B0r',

},

[7] = { -- table(cc1df9d)

['address'] = 0xca271cc6,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '8Dr',

},

[8] = { -- table(11c3c12)

['address'] = 0xca271cc7,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'E2r',

},

})
gg.clearResults()
gg.toast("Mode Ghost OFF👻❌")
gg.setVisible(false)
end

function d5()
gg.clearResults()
gg.setRanges(16392)

gg.getRangesList("libunity.so")

gg.searchNumber("7Fr;45r;4Cr;46r;01r;01r;01r;00r::8", 1, false, 536870912, 3462987776, 3483348992)

gg.getResultsCount()

gg.getResults(8)

gg.loadResults({ -- table(84d5341)

[1] = { -- table(d9070e6)

['address'] = 0xcf582890,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 127,

},

[2] = { -- table(ca5627)

['address'] = 0xcf582891,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 69,

},

[3] = { -- table(590c0d4)

['address'] = 0xcf582892,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 76,

},

[4] = { -- table(15ae27d)

['address'] = 0xcf582893,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 70,

},

[5] = { -- table(e62b572)

['address'] = 0xcf582894,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[6] = { -- table(b7c2dc3)

['address'] = 0xcf582895,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[7] = { -- table(1a31640)

['address'] = 0xcf582896,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[8] = { -- table(8b82979)

['address'] = 0xcf582897,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 0,

},

})

gg.setValues({ -- table(fc3566c)

[1] = { -- table(e08e435)

['address'] = 0xcf582890,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'C9r',

},

[2] = { -- table(54f80ca)

['address'] = 0xcf582891,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '3Cr',

},

[3] = { -- table(253923b)

['address'] = 0xcf582892,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '8Er',

},

[4] = { -- table(8c56d58)

['address'] = 0xcf582893,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'BFr',

},

[5] = { -- table(d708eb1)

['address'] = 0xcf582894,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'C9r',

},

[6] = { -- table(24cdf96)

['address'] = 0xcf582895,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '3Cr',

},

[7] = { -- table(adf1717)

['address'] = 0xcf582896,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '8Er',

},

[8] = { -- table(cd00704)

['address'] = 0xcf582897,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'BFr',

},

})

gg.clearResults()

gg.setRanges(16392)

gg.searchNumber("7Fr;45r;4Cr;46r;01r;01r;01r;00r::8", 1, false, 536870912, 3462987776, 3483348992)

gg.getResultsCount()

gg.getResults(8)

gg.loadResults({ -- table(3453934)

[1] = { -- table(e1f635d)

['address'] = 0xcf582898,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 127,

},

[2] = { -- table(a91d4d2)

['address'] = 0xcf582899,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 69,

},

[3] = { -- table(b5bc9a3)

['address'] = 0xcf58289a,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 76,

},

[4] = { -- table(496b4a0)

['address'] = 0xcf58289b,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 70,

},

[5] = { -- table(2f3d859)

['address'] = 0xcf58289c,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[6] = { -- table(b946c1e)

['address'] = 0xcf58289d,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[7] = { -- table(4575cff)

['address'] = 0xcf58289e,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[8] = { -- table(3c27acc)

['address'] = 0xcf58289f,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 0,

},

})

gg.setValues({ -- table(a0277b8)

[1] = { -- table(3d73991)

['address'] = 0xcf582898,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'C9r',

},

[2] = { -- table(d53e0f6)

['address'] = 0xcf582899,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '3Cr',

},

[3] = { -- table(f85ecf7)

['address'] = 0xcf58289a,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '8Er',

},

[4] = { -- table(8435764)

['address'] = 0xcf58289b,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'BFr',

},

[5] = { -- table(f551682)

['address'] = 0xcf58289c,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'C9r',

},

[6] = { -- table(ea26193)

['address'] = 0xcf58289d,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '3Cr',

},

[7] = { -- table(f9085d0)

['address'] = 0xcf58289e,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '8Er',

},

[8] = { -- table(82e89c9)

['address'] = 0xcf58289f,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'BFr',

},

})
gg.clearResults()
gg.toast("Bug Batu Aktiff✔")
gg.setVisible(false)
end

function d6()
gg.clearResults()
gg.setRanges(16392)

gg.searchNumber("7Fr;45r;4Cr;46r;01r;01r;01r;00r::8", 1, false, 536870912, 3462987776, 3483348992)

gg.getResultsCount()

gg.getResults(8)

gg.loadResults({ -- table(289219e)

[1] = { -- table(ad5a47f)

['address'] = 0xcf582890,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 127,

},

[2] = { -- table(e4fec4c)

['address'] = 0xcf582891,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 69,

},

[3] = { -- table(cef495)

['address'] = 0xcf582892,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 76,

},

[4] = { -- table(6e1b9aa)

['address'] = 0xcf582893,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 70,

},

[5] = { -- table(82a099b)

['address'] = 0xcf582894,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[6] = { -- table(1b58138)

['address'] = 0xcf582895,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[7] = { -- table(f74511)

['address'] = 0xcf582896,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[8] = { -- table(c79c676)

['address'] = 0xcf582897,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 0,

},

})

gg.setValues({ -- table(e7a214d)

[1] = { -- table(eb3d402)

['address'] = 0xcf582890,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'BDr',

},

[2] = { -- table(8b61113)

['address'] = 0xcf582891,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '37r',

},

[3] = { -- table(66b3f50)

['address'] = 0xcf582892,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '86r',

},

[4] = { -- table(d48549)

['address'] = 0xcf582893,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '35r',

},

[5] = { -- table(a8f2e4e)

['address'] = 0xcf582894,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'BDr',

},

[6] = { -- table(d66ab6f)

['address'] = 0xcf582895,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '37r',

},

[7] = { -- table(3cd007c)

['address'] = 0xcf582896,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '86r',

},

[8] = { -- table(572d05)

['address'] = 0xcf582897,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '35r',

},

})

gg.clearResults()

gg.setRanges(16392)

gg.searchNumber("7Fr;45r;4Cr;46r;01r;01r;01r;00r::8", 1, false, 536870912, 3462987776, 3483348992)

gg.getResultsCount()

gg.getResults(8)

gg.loadResults({ -- table(1566175)

[1] = { -- table(8f7550a)

['address'] = 0xcf582898,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 127,

},

[2] = { -- table(cf4717b)

['address'] = 0xcf582899,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 69,

},

[3] = { -- table(ec9fb98)

['address'] = 0xcf58289a,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 76,

},

[4] = { -- table(6389ff1)

['address'] = 0xcf58289b,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 70,

},

[5] = { -- table(c3db7d6)

['address'] = 0xcf58289c,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[6] = { -- table(9ceaa57)

['address'] = 0xcf58289d,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[7] = { -- table(b2f3944)

['address'] = 0xcf58289e,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 1,

},

[8] = { -- table(5264a2d)

['address'] = 0xcf58289f,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 0,

},

})

gg.setValues({ -- table(188a5b0)

[1] = { -- table(fca5c29)

['address'] = 0xcf582898,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'BDr',

},

[2] = { -- table(1f3ebae)

['address'] = 0xcf582899,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '37r',

},

[3] = { -- table(d3e8d4f)

['address'] = 0xcf58289a,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '86r',

},

[4] = { -- table(cca6cdc)

['address'] = 0xcf58289b,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '35r',

},

[5] = { -- table(d8391e5)

['address'] = 0xcf58289c,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = 'BDr',

},

[6] = { -- table(39714ba)

['address'] = 0xcf58289d,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '37r',

},

[7] = { -- table(180af6b)

['address'] = 0xcf58289e,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '86r',

},

[8] = { -- table(ff7ac8)

['address'] = 0xcf58289f,

['flags'] = 1, -- gg.TYPE_BYTE

['value'] = '35r',

},

})
gg.clearResults()
gg.toast("Bug Batu OFF❌")
gg.setVisible(false)
end

function d7()
gg.setRanges(8)
gg.searchNumber("BDr;37r;86r;35r;00r;00r;A0r;E3r::8", 1, false, 536870912, 0, -1)
gg.getResults(8)
gg.editAll("4Ar;EBr;8Fr;BFr;00r;00r;A0r;E3r", 1)
gg.clearResults()
gg.toast("ᴡᴀʟʟʜᴀᴄᴋ ᴏɴ")
end

function d8()
gg.setRanges(8)
gg.searchNumber("4Ar;EBr;8Fr;BFr;00r;00r;A0r;E3r::8", 1, false, 536870912, 0, -1)
gg.getResults(8)
gg.editAll("BDr;37r;86r;35r;00r;00r;A0r;E3r", 1)
gg.clearResults()
gg.toast("ᴡᴀʟʟʜᴀᴄᴋ ᴏғғ")
end

function d9()
gg.setRanges(32)
gg.searchNumber("3.25;1.4;1.9;0.8::13", 16, false, 536870912, 0, -1)
gg.getResults(4)
gg.editAll("4.8;1.4;2.8;1.2", 16)
gg.toast("sᴘᴇᴇᴅ ʀᴜɴ 𝟻x ᴏɴ")
end

function d10()
gg.setRanges(32)
gg.searchNumber("4.8;1.4;2.8;1.2", 16, false, 536870912, 0, -1)
gg.getResults(4)
gg.editAll("3.25;1.4;1.9;0.8::13", 16)
gg.toast("sᴘᴇᴇᴅ ʀᴜɴ 𝟻x ᴏFF")
end

function d11()
gg.setRanges(32)
gg.searchNumber("1Fr;85r;ABr;3Fr;00r;00r;80r;3Fr;00r;00r;80r;3Fr::12", 1, false, 536870912, 0, -1)
gg.getResults(12)
gg.editAll("1Fr;85r;ABr;3Fr;00r;00r;90r;41r;00r;00r;80r;3Fr", 1)
gg.clearResults()
gg.toast("Eagle Eye ON")
end

function d12()
gg.setRanges(32)
gg.searchNumber("1Fr;85r;ABr;3Fr;00r;00r;90r;41r;00r;00r;80r;3Fr::12", 1, false, 536870912, 0, -1)
gg.getResults(12)
gg.editAll("1Fr;85r;ABr;3Fr;00r;00r;80r;3Fr;00r;00r;80r;3Fr", 1)
gg.clearResults()
gg.toast("Eagle Eye Off")
end
0
function Exit()

print("E N J O Y I N W O R L D")

os.exit()

end

function HOME()

lw=1

Main()

end

cs = '这里可以填QQ'

while(true)do

if gg.isVisible(true) then

XGCK=1

gg.setVisible(false)
end

gg.clearResults()

if XGCK==1 then

Main()

end

end
